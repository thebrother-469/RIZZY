-- =========================================================================
-- 1. Subscriptions: remove client write access. Only service role writes.
-- =========================================================================
DROP POLICY IF EXISTS "own sub insert" ON public.subscriptions;
DROP POLICY IF EXISTS "own sub update" ON public.subscriptions;
-- keep "own sub read" (SELECT of own row)

-- Belt-and-suspenders trigger: reject any non-service_role mutation.
...
CREATE TRIGGER subscriptions_guard_ins BEFORE INSERT ON public.subscriptions
  FOR EACH ROW EXECUTE FUNCTION public.subscriptions_guard();
CREATE TRIGGER subscriptions_guard_upd BEFORE UPDATE ON public.subscriptions
  FOR EACH ROW EXECUTE FUNCTION public.subscriptions_guard();
CREATE TRIGGER subscriptions_guard_del BEFORE DELETE ON public.subscriptions
  FOR EACH ROW EXECUTE FUNCTION public.subscriptions_guard();

REVOKE EXECUTE ON FUNCTION public.subscriptions_guard() FROM PUBLIC, anon, authenticated;

-- =========================================================================
-- 2. Gamification: SELECT-only for authenticated, no client writes.
--    Missions keep DELETE for the account-wipe flow.
-- =========================================================================
...
-- =========================================================================
-- 3. SECURITY DEFINER RPCs — whitelisted, server-authoritative writes.
-- =========================================================================

-- XP reward whitelist (matches src/lib/xp.ts XP_REWARDS)
...
BEGIN
  ...
  -- level curve: level N requires 100*N XP
  LOOP
    _needed := 100 * _level;
...
-- Badge whitelist (matches src/lib/xp.ts BADGES)
...
BEGIN
  ...
  IF _key NOT IN (
    ...
    'level_5','level_10','first_roast','memory_master'
  ) THEN
...
-- Mission completion: marks complete, advances streak, returns state.
-- XP/badges are awarded by the client via award_xp/award_badge (both
-- whitelisted), matching existing behavior.
...
-- =========================================================================
-- 4. Lock down function EXECUTE privileges.
-- =========================================================================
